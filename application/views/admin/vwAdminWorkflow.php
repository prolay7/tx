...
<!--link rel="stylesheet" href="<?php echo HTTP_ASSETS_PATH_ADMIN; ?>css/jquery-ui.css"/>
 <script src="<?php echo HTTP_ASSETS_PATH_ADMIN; ?>js/jquery-ui.custom.js"></script>
    ...
    <!--link rel="stylesheet" href="<?php echo HTTP_ASSETS_PATH_ADMIN; ?>jfiller/jquery.fileuploader.css"/>
    <script src="<?php echo HTTP_ASSETS_PATH_ADMIN; ?>jfiller/jquery.fileuploader.min.js"></script-->
						...
						?>
							<table class="table table-striped table-bordered sticky_column" id="table_id" style="width:auto !important;">
								<thead>
									...
									<th style = "text-align: center;width:70px;min-width:70px;max-width:70px;">Line No
									...
									<th style="width: <?php echo $_width;?>min-width:<?php echo $_width;?>max-width:<?php echo $_width;?>">Job
									...
									<th style = " text-align: center;width:150px;min-width:150px;max-width:150px;" class="<?php echo ($show_hide_stages[0]->show_hide==2)?'hidecol':'';?>">Date Recieved</th>
									<th style = " text-align: center;width:150px;min-width:150px;max-width:150px;" class="<?php echo ($show_hide_stages[0]->show_hide==2)?'hidecol':'';?>">Due Date</th>
									<th style = "text-align: center;width:150px;min-width:150px;max-width:150px;" class="<?php echo ($show_hide_stages[0]->show_hide==2)?'hidecol':'';?>">Translator Name</th>
									<th style = "text-align: center;width:150px;min-width:150px;max-width:150px;" class="<?php echo ($show_hide_stages[0]->show_hide==2)?'hidecol':'';?>">Language From</th>
									<th style = "text-align: center;width:150px;min-width:150px;max-width:150px;" class="<?php echo ($show_hide_stages[0]->show_hide==2)?'hidecol':'';?>">Language To</th>
									<div id="stages_head">
									<?php
									...
									if(!empty($wstages)){
										...
										foreach ($wstages as $key => $value) {
											//if(!in_array($value->stage_column,$exclude_head)){
												...
												?>
												<th style = "width: 150px;min-width:150px;max-width:150px;text-align: center;background:<?php echo $background;?>;color:<?php echo $textcolor;?>" onclick="change_stage_data(<?php echo $cname[0]->wid;?>);"><?php echo $cname[0]->stage_name;?></th>
												<?php
											//}
										}
									}
											...
											foreach ($jobs as $key => $value) {
												...
												?>
													<tr align="center">
														...
														<td class="" style="width: <?php echo $_width;?>min-width:<?php echo $_width;?>"><span class="pull-left"><?php echo $value->name;?></span>
														<br>
															<a href="javascript:void(0);" data-toggle="modal" data-target="#myModal4" data-jobid="<?php echo $value->id;?>" data-lineno="<?php echo $value->lineNumber;?>" data-jobname="<?php echo $value->name;?>" class="mark" id="mark<?php echo $value->lineNumber;?>"><?php echo $compliant_msg;?></a>
																...
																//if(!in_array($value2->stage_column,$exclude_head)){
							                                       ...
							                                       // print_r($attributes);
																	if(!empty($celldata) && $celldata[0]->stage_data!=''){
																		...
																		<select class="form-control selector" style="font-size: 10px;font-weight: 600; height: 24px;padding: 2px 5px;background:<?php echo $cell_background;?>;color:<?php echo $textcolor;?>" data-linenumber="<?php echo $value->lineNumber;?>" data-id="<?php echo $cname[0]->wid;?>" data-key="<?php echo $cname[0]->stage_column;?>" data-cell="<?php echo $value2->stage_column.$count2.$value->id;?>" data-dataid="<?php echo $value->id;?>" data-stageorder="<?php echo $cname[0]->stage_order;?>" data-stagecolor="<?php echo $cname[0]->stage_color;?>"	data-stagename="<?php echo $cname[0]->stage_name;?>" data-stagetextcolor="<?php echo $cname[0]->stage_text_color;?>">
			...
			<div class="modal-body">
	              ...
	              <div class="form-group">
	              	<div class="checkbox checkbox-danger">
                        <input id="priority_high" type="checkbox" checked="" name="stage_priority" value="1">
...
<?php
/*echo '<pre>';
print_r($this->workflow_model->get_prev_priority(5));
echo '</pre>';*/

/*$arr=$this->workflow_model->get_data_by_line_data_id(3,2,'C');
...
print_r($this->workflow_model->get_min_wid()[0]->min_wid);
...
<div class="modal fade" id="myModal4" tabindex="-1" role="dialog" aria-labelledby="modalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
	<div class="modal-content">
		<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span><span class="sr-only">Close</span></button>
			<h3 class="modal-title" id="lineModalLabel">Client Compliants</h3>
		</div>

		<form enctype="multipart/form-data" action="" method="post" id="stageform23">
			...
			<div class="modal-body">
				...
				<div class="col-md-6">
	              <div class="form-group">
	                ...
	                <input type="text" class="form-control" id="job_name" disabled="disabled">
	                <input type="hidden" class="form-control" id="job_name2" name="job_name">
	                <input type="hidden" class="form-control" id="job_id" name="job_id">
	              </div>
	              <div class="form-group">
	              	<label for="exampleInputEmail1">Mark as</label>
	              	<select class="form-control" name="stage_mark" id="stage_mark">
	              		<option value="1">Edit</option>
	              		<option value="2">Error</option>
	              		<!--option value="3">Edited/Error Free</option-->
	              	</select>
	              </div>
	              <div class="form-group">
	              	<label for="exampleInputEmail1">Client Compliant</label>
	              	<textarea class="form-control" rows="5" cols="10" name="compliant" id="compliant"></textarea>
	              </div>
	              <div class="form-group">
	              	<label for="exampleInputEmail1">Additional Comments</label>
	              	<textarea class="form-control" rows="5" cols="10" name="compliant_reply" id="compliant_reply"></textarea>
...
$(document).on('click','.mark',function(e){
		...
		$('#myModal4').find('#line_no').val($(this).attr('data-lineno'));
		$('#myModal4').find('#line_no2').val($(this).attr('data-lineno'));
		$('#myModal4').find('#job_name').val($(this).attr('data-jobname'));
		$('#myModal4').find('#job_name2').val($(this).attr('data-jobname'));
		$('#myModal4').find('#job_id').val($(this).attr('data-jobid'));
		$('#myModal4').find('#job_id2').val($(this).attr('data-jobid'));
...
$(document).ready(function(){
	...
	/*$('.mark').on('click',function(){
		...
		/*$('input[name="files"]').fileuploader({
			...
			extensions: ['jpg','jpeg','png','gif','pdf','doc','docx','ppt','pptx','odt','txt','xls','xlsx','zip','rar'],
			onItemShow: function(item, listEl, parentEl, newInputEl, inputEl) {
				$('#selectedfiles').val(item);
				files=$('fileuploader-list-files').val();
				console.log(item);
			}
		});*/


	$('#stageform23').validate({
		rules:{
			compliant:{
				required:true
			},
		...
		messages:{
			compliant:{
				required:'Enter Client compliant'
			},
			compliant_reply:{
	...
	$('#saveImage24').on('click',function(){
		$.ajax({
			type:'POST',
			url:'<?php echo base_url()."admin_workflow/upload_files";?>',
			data:$('#myModal4').find('#stageform24').serialize(),
			success:function(data){
				console.log(data);
			}
		});
	...
	/*$('#saveImage23').on('click', function() {
    var file_data = $('#images').prop('files')[0];   
    var form_data = new FormData();                  
    form_data.append('file', file_data);
    alert(form_data);                             
    $.ajax({
                url: '<?php echo base_url()."admin_workflow/mark_on_job2";?>', // point to server-side PHP script 
				...
				$('#saveImage23').on('click', function () {
					 var valid=$('#stageform23').valid();
					 if(valid){
	                    var form_data = new FormData();
	                    var ins = document.getElementById('images').files.length;
	                    var other_data = $('#myModal4').find('form').serialize();
	                    for (var x = 0; x < ins; x++) {
	                        form_data.append("files[]", document.getElementById('images').files[x]);
	                    }
	                  ...
	                  //  data_string='compliant='+document.getElementById('compliant').value+'&compliant_reply='+document.getElementById('compliant_reply').value+'&line_no='+document.getElementById('line_no').value+'&stage_mark='+document.getElementById('stage_mark').value+'&job_name',document.getElementById('job_name').value+'&job_id='+document.getElementById('job_id').value;
	                    $.ajax({
	                        url: '<?php echo base_url()."admin_workflow/mark_on_job2?";?>'+other_data, // point to server-side PHP script 
...
$('#saveImage234').on('click',function(){

		 var valid=$('#stageform23').valid();
		if(valid){
			console.log($('#stageform23').serialize());
			//var formData = new FormData( $("#stageform23")[0] );
			$.ajax({
				type:'POST',
				url:'<?php echo base_url()."admin_workflow/mark_on_job";?>',
				data : $('#stageform23').serialize(),
				beforeSend:function(){
					$('#saveImage23').append(" <span class='fa fa-spinner fa-spin'></span>");
				},
				success:function(data){
					console.log(data);
					...
					if(json.hasOwnProperty('success')){
						...
						$('#mark'+$('#line_no').val()).html(json.marked);
						$('#msgs').html(json.success);
						$('#saveImage23 span.fa').remove();

						setTimeout(function(){
							$('#msgs').html('');
							//$('#stageform23').css('display','none');
							//$('#stageform24').css('display','block');
							$("#myModal4 .close").click();
						},1500);
					}else{
						$('#msgs').html(json.error);
						$('#saveImage23 span.fa').remove();
	...
	$('#myModal4').on('shown.bs.modal', function (e) {
		//$('.modal-backdrop').remove();
	 		...
	 		var lineno=$(this).find('#line_no').val();
	 		$.ajax({
	 			type:'GET',
	 			url:'<?php echo base_url().'admin_workflow/load_compliants/';?>'+lineno,
	 			data:{},
	 			beforeSend:function(){
	 			...
	 			success:function(data){
	 				...
	 				$('#myModal4').find('textarea#compliant').html(json.compliant);
	 				$('#myModal4').find('textarea#compliant_reply').html(json.compliants_reply);
	 				if(json.marked_as==null){
	 					$('#myModal4').find("#stage_mark").val(1).trigger('change');
	 				}else{
	 					$('#myModal4').find("#stage_mark").val(json.marked_as).trigger('change');
	 				}
	...
	$("#myModal4").on("hidden.bs.modal", function () {
	    $('#stageform23').trigger('reset');
	    $('#stageform23').css('display','block');
		$('#stageform24').css('display','none');
	});
	//13-07-17
...
//17-05-17
	$(document).on('change', '.selector', function (e) { 
		...
		var stage_linenumber=$(this).attr('data-linenumber');
		var reorderd=$('#reorderd').val();
		//console.log(stage_text_color);

		console.log('wid='+wid+'key='+key+'cell='+cell+'data_id='+data_id+'stage_order='+stage_order+'stage_data='+stage_data+'stage_color='+stage_color+'stage_name='+stage_name+'data_line_number='+stage_linenumber);

	 
	  $.ajax({
			type:'POST',
			url:'<?php echo base_url().'admin_workflow/update_stage_cell';?>',
			data:{wid:wid,key:key,cell:cell,data_id:data_id,stage_order:stage_order,stage_data:stage_data,stage_color:stage_color,stage_name:stage_name,reorderd:reorderd,stage_order:stage_order,stage_text_color:stage_text_color,stage_linenumber:stage_linenumber},
			success:function(data){
				json=$.parseJSON(data);
				console.log(data);
				if(json.hasOwnProperty('success')){
...
function change_stage_cell_data(wid,key,cell,data_id,stage_order,stage_data,stage_color,stage_name,reorderd){
	...
	//11-07-17
		function change_stage_priority(id,data){
			$.ajax({
				type:'GET',
				url:'<?php echo base_url().'admin_workflow/change_stage_priority/';?>'+id+'/'+data,
				data:{},
				beforeSend:function(){
					$('#'+id+'spans').css('display','block');
				},
				success:function(data){
						$('#'+id+'spans').css('display','block').html('Priority High');
						setTimeout(function(){
							load_stage();
							$('#'+id+'spans').css('display','none').html('');
						},1500);
...
.radio-success input[type="radio"]:checked + label::after {
  background-color: #5cb85c; }
  mark, .mark{ background-color: transparent; float: left; margin-top: -3px;}	
</style>
