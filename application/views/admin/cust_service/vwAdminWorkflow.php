<?php $this->load->view('admin/cust_service/includes/vwHeader'); ?>
...
<div id="main-container" class="main-container">
	...
	<?php	$this->load->view('admin/cust_service/includes/vwSidebar-left'); ?>
			...
			<div class="page-content">
				...
				<div class="row">
					<div class="design-form" style="width:100%">
                            ...
                            echo form_open('cs_adminworkflow/workflow/' . $job, $attributes);
                            ...
                            echo form_submit($data_submit);
                         	?>
                         		<input type="button" class="btn btn-info btn-sm" onclick="window.location.href='<?php echo base_url().'cs_adminworkflow/reset';?>'" value="Reset">
                         	<?php
                            echo form_close();
						...
						?>
							<table class="table table-striped table-bordered sticky_column" id="table_id" style="width:auto !important;">
								<thead>
									...
									<th style = " text-align: center; max-width:150px;width:150px;min-width:150px;" class="<?php echo ($show_hide_stages[0]->show_hide==2)?'hidecol':'';?>">Date Recieved</th>
									<th style = " text-align: center;  max-width:150px;width:150px;min-width:150px;" class="<?php echo ($show_hide_stages[0]->show_hide==2)?'hidecol':'';?>">Due Date</th>
...
$(document).ready(function(){
	$('.popup').popover({
      ...
      $.ajax({
      	type:'GET',
      	url:'<?php echo base_url().'cs_adminworkflow/load_saved_notes/';?>'+$(this).attr('data-id'),
      	data:{},
      ...
      $('#btnsave').click(function(){
	      $.ajax({
	      	type:'POST',
	      	url:'<?php echo base_url().'cs_adminworkflow/save_notes';?>',
...
//17-05-17
	$(document).on('change', '.selector', function (e) { 
	  ...
	  $.ajax({
			type:'POST',
			url:'<?php echo base_url().'cs_adminworkflow/update_stage_cell';?>',
			...
			success:function(data){
				...
				if(json.hasOwnProperty('success')){
					...
					$("#editabletable").load('<?php echo base_url().'cs_adminworkflow/load_stages/'.$this->uri->segment(3);?>');
				}else if(json.hasOwnProperty('error')){
					alert(json.error);
				}
			}
		...
		$(document).on('click','#show',function(e){
			...
			$.ajax({
				type:"GET",
				url:'<?php echo base_url().'cs_adminworkflow/show_hide_stages/1';?>',
				data:{},
				success:function(data){
					//window.location.href='<?php echo base_url().'admin/workflow';?>';
					$("#editabletable").load('<?php echo base_url().'cs_adminworkflow/load_stages';?>');
				}
			});
		});
		...
		$(document).on('click','#hide',function(e){
			...
			$.ajax({
				type:"GET",
				url:'<?php echo base_url().'cs_adminworkflow/show_hide_stages/2';?>',
				data:{},
				success:function(data){
					//window.location.href='<?php echo base_url().'admin/workflow';?>';
					$("#editabletable").load('<?php echo base_url().'cs_adminworkflow/load_stages';?>');
...
function change_stage_cell_data(wid,key,cell,data_id,stage_order,stage_data,stage_color,stage_name,reorderd){
	...
	function change_stage_id(id){
		...
		$.ajax({
			type:'GET',
			url:'<?php echo base_url().'cs_adminworkflow/get_stage_data/';?>'+id,
			data:{},
			success:function(data){
				console.log(data);
				json=$.parseJSON(data);
	...
	function load_stage(){
		$('#myModal2').find('#SortMe').load('<?php echo base_url().'cs_adminworkflow/loadstages' ?>');
		$('#myModal3').find('#SortMe3').load('<?php echo base_url().'cs_adminworkflow/loadstages' ?>');
	}

	function load_stage_attributes(id){
		/*$('#myModal3').find('#stage_attributes').load('<?php echo base_url().'cs_adminworkflow/load_stage_attributes/' ?>'+id);
		$('#myModal3').find('#stagelist23').css('display','block');*/

		$.ajax({
			type:'GET',
			url:'<?php echo base_url().'cs_adminworkflow/load_stage_attributes/' ?>'+id,
			data:{},
			success:function(data){
				if(data!=''){
	...
	function add_stage_attribute(){
		...
		if(attr_name!=''){
			$.ajax({
				type:'POST',
				url:'<?php echo base_url().'cs_adminworkflow/add_new_stage_attribute';?>',
				...
				success:function(data){
					...
					setTimeout(function(){$('#myModal3').find('#msg3').html('');load_stage_attributes(stage_id);$("#editabletable").load('<?php echo base_url().'cs_adminworkflow/load_stages';?>');$('#myModal3').find('#stagelist23').css('display','block');$('#myModal3').find('#attr_name').val('');},2000);	
	...
	function update_stage_attribute(id){
		...
		if(attr_name!='' && attr_id!=''){
			$.ajax({
				type:'POST',
				url:'<?php echo base_url().'cs_adminworkflow/update_stage_attribute';?>',
	...
	function delete_stage_attribute(id,stage_id){
		...
		$.ajax({
			type:'GET',
			url:'<?php echo base_url().'cs_adminworkflow/delete_stage_attribute/' ?>'+id+'/'+stage_id,
			success:function(data){
				json=$.parseJSON(data);
				if(json.hasOwnProperty('success')){
					$('#myModal3').find('#msg3').html('<div class="alert alert-success">'+json.success+'</div>');
					load_stage_attributes($('#myModal3').find('#column_id3').val());
					setTimeout(function(){$('#myModal3').find('#msg3').html('');$("#editabletable").load('<?php echo base_url().'cs_adminworkflow/load_stages';?>');$('#myModal3').find('#stagelist23').css('display','block');},2000);
					if(attr_name!='' && attr_id!=''){
						$('#myModal3').find('#attr_id').val('');
...
$('#stageform').validate({
	...
	submitHandler:function(form){
		$.ajax({
			type:'POST',
			url:'<?php echo base_url().'cs_adminworkflow/add_stage';?>',
...
function delete_stage(id){
	...
	$.ajax({
		type:'GET',
		url:'<?php echo base_url().'cs_adminworkflow/delete_stage/';?>'+id,
		data:{},
		beforeSend:function(){
...
$(document).ready(function () {
     ...
     $('#SortMe3').sortable({
         ...
         update: function (event, ui) {
             ...
             $.ajax({
             	type:'POST',
             	url:'<?php echo base_url().'cs_adminworkflow/change_order';?>',
             	data:$('#stageform3').serialize(),
             	success:function(data){
             		console.log(data);
             		$("#editabletable").load('<?php echo base_url().'cs_adminworkflow/load_stages';?>');
