...
Class Admin_Invoice extends CI_Controller
{
    ...
    {
        $o=$this->session->userdata('order_by_invoice');
        $ses=!empty($o)?$this->session->userdata('order_by_invoice'):'bidjob.complete_date ASC';
        $this->session->set_userdata('order_by_invoice',$ses);
...
//        print($ordet_by);exit();
        if(isset($ordet_by) == false || $ordet_by== ''){
            $ordet_by = 'bidjob.complete_date DESC';
        }
...
//        print_r($this->input->post());exit();
    ...
    public function sort(){
        if($this->input->server('REQUEST_METHOD') == 'POST' && $this->input->is_ajax_request() == true){
            $this->session->set_userdata('order_by_invoice','');
